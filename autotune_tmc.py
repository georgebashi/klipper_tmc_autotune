...
class AutotuneTMC:
    ...
    def _get_tblank_cycles(self, tbl):
        if self.driver_type in ["tmc2208", "tmc2209", "tmc2240"]:
            tbl_cycles = [16, 24, 32, 40]
        else:
            tbl_cycles = [16, 24, 36, 54]
        return tbl_cycles[tbl]

    def _set_hysteresis(self, run_current):
        hstrt, hend = self.motor_object.hysteresis(
            tblank=self._get_tblank_cycles(self.tbl) / self.fclk,
            volts=self.voltage,
            current=run_current,
            toff=self.toff,
            fclk=self.fclk,
    ...
    def _setup_spreadcycle(self):
        ...
        pfdcycles = ncycles - (24 + 32 * self.toff) * 2 - self._get_tblank_cycles(self.tbl)
